...
from organiser import build_plan, _safe_copy
from state import State
from deduper import _md5
...
def main():
    parser = argparse.ArgumentParser(description="Local Media Organiser")
    ...
    try:
        ...
        seen_count = len(state.data["processed"])
        if seen_count:
            info(f"State loaded — {seen_count} files already seen from previous runs")

        # ── Step 1: Scan ───────────────────────────────────────
        hdr("Step 1: Scanning files")
        all_records = scan_folder(args.input)

        # Filter out already-seen files using MD5
        new_records = []
        skipped = 0
        for rec in tqdm(all_records, desc="Checking state", unit="file"):
            if rec.file_type in ("image", "screenshot", "video", "document", "other"):
                md5 = _md5(rec.path)
                if state.is_seen(md5):
                    skipped += 1
                else:
        ...
        ok(f"Found {len(all_records)} files total")
        if skipped:
            info(f"Skipped {skipped} already-organised files")
        ok(f"Processing {len(new_records)} new files")
        ...
        if not new_records:
            ok("Nothing new to organise — all files already processed.")
            return
        ...
        if args.no_dedup:
            ...
            hdr("Step 2: Duplicate detection")
            exact_dupes, near_dupes_raw = find_duplicates(new_records)
        ...
        # ── Step 3: Face clustering ────────────────────────────
        face_clusters = {}
        if args.skip_faces:
            hdr("Step 3: Face clustering")
            info("Skipped (--skip-faces)")
        else:
            hdr("Step 3: Face clustering")
            face_clusters = cluster_faces(new_records, state)
        ...
        # ── Step 4: Build plan ─────────────────────────────────
        hdr("Step 4: Building organisation plan")
        plan = build_plan(new_records, exact_dupes, near_dupes, face_clusters, args.output)
        ...
        if args.dry_run:
            ...
            for move in tqdm(plan.moves, desc="Copying files", unit="file"):
                ...
                if _safe_copy(move.src, move.dst):
                    ...
                    md5 = _md5(move.src)
                    state.mark_seen(md5, str(move.src), str(move.dst))
                else:
                    results["failed"] += 1
            ...
            ok(f"State saved → {state.state_path}")
            if results["failed"]:
